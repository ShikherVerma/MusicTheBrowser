...
UntrustedSnapHostUI::UntrustedSnapHostUI(content::WebUI* web_ui)
  ...
  // 'unsafe-inline' is intentionally omitted — snap_host.html loads one
  ...
  // These three restate chrome-untrusted defaults (frame-src and worker-src
  // via child-src 'none'). Spelled out because this frame evaluates untrusted
  // code, so they must not silently relax if the defaults change upstream.
  untrusted_source->OverrideContentSecurityPolicy(
