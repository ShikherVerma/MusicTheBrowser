...
// down to its own first-party bundle plus whatever `frame_src`,
// `frame_ancestors` and `connect_src` (full CSP directive values) allow.
//
...
void AddViewerDataSource(content::WebUI* web_ui,
  ...
  source->OverrideContentSecurityPolicy(
      network::mojom::CSPDirectiveName::ConnectSrc, std::string(connect_src));
  source->OverrideContentSecurityPolicy(
...
// LeoWorkspaceViewUI) control viewer documents, which is what makes their
// /files/<path> fetches relayable to the parent workspace frame. Workspace
// documents stay uncontrolled: no worker is registered for a workspace origin.
...
LeoWorkspaceViewUI::LeoWorkspaceViewUI(content::WebUI* web_ui, const GURL& url)
  ...
  // The page cannot fetch the workspace's files itself; it gets them through
  // its service worker, which serves them as same-origin /files/<path> URLs.
...
// viewer creation picks up worker updates; a worker that is already
// up-to-date is a no-op. The worker starts controlling documents once it
// activates, so fetches made before then fall through to the data source.
