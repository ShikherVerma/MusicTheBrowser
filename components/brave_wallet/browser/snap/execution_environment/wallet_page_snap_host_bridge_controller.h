...
// SnapHostBridgeController implementation that uses an already-open wallet
// page as the snap isolation environment. Does not open a page; IsBound() is
// true while a SnapHostBridge remote is bound (the page creates the pipe and
// passes the remote end).
//
// BindNewBridge is last-wins across the profile: a second wallet tab replaces
// the first tab's bridge remote. Two consequences, acceptable while snaps are
