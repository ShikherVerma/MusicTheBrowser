 ...
 public:
  ...
  void UnlockWallet() {
    ...
    keyring_service()->Unlock(
        kTestWalletPassword, base::BindLambdaForTesting([&](bool success) {
          ASSERT_TRUE(success);
          run_loop.Quit();
        }));
    run_loop.Run();
...
// Only the mainnet and import keyrings are dapp-visible; a testnet account
// does not make the origin connectable.
TEST_F(PolkadotProviderImplUnitTest,
...
// The candidate list handed to the permission layer is what the prompt shows,
// so it must be every dapp-visible account rather than just the selected one.
TEST_F(PolkadotProviderImplUnitTest,
  ...
  ASSERT_TRUE(first && second && imported);

  EXPECT_CALL(*delegate(),
              GetAllowedAccounts(
                  mojom::CoinType::DOT,
                  testing::UnorderedElementsAre(PermissionIdentifier(first),
...
TEST_F(PolkadotProviderImplUnitTest, Enable_AlreadyPermitted) {
  ...
  // Already granted, so the user is not prompted again.
...
// The remote `Enable` resolves with must be a live `PolkadotApiImpl` bound to
// the granted account, not just a non-null handle.
TEST_F(PolkadotProviderImplUnitTest, Enable_GrantedRemoteServesTheAccount) {
  ...
  // The api impl was handed its own delegate, which still gates every call.
...
// The provider is only injected once a wallet exists, but a reset while the
// page stays open must still fail cleanly instead of crashing.
...
// A grant naming an account we do not hold resolves to no preferred account,
// which must be a rejection rather than a null-account remote.
...
TEST_F(PolkadotProviderImplUnitTest, Enable_NoAccounts_ShowsAccountCreation) {
  ...
  EXPECT_CALL(*delegate(), ShowAccountCreation(mojom::CoinType::DOT,
                                              url::Origin::Create(
                                                  GURL(kTestOrigin))))
      .Times(1);
...
// The account creation prompt is latched, so a dapp calling `enable()` in a
// loop cannot reopen it.
TEST_F(PolkadotProviderImplUnitTest,
...
// Only one request can wait on the unlock; a second is rejected rather than
...
TEST_F(PolkadotProviderImplUnitTest,
       Enable_WalletLocked_ResolvesAfterUnlock) {
  CreateWallet();
  auto account = AddAccount();
  ASSERT_TRUE(account);
  SetAllowedAccounts({PermissionIdentifier(account)});
...
// The parked request is re-evaluated on unlock rather than assumed to succeed,
// so a permission that is no longer granted still rejects.
TEST_F(PolkadotProviderImplUnitTest,
